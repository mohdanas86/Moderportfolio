...
const TechIcon = ({ icon, className = "size-6" }) => {
    if (!icon) return null
    ...
    if (icon?.src || icon?.icon) {
        const iconSrc = icon.src || icon.icon
 ...
 * @param {Array} techs - Array of tech icons (can be strings, URLs, components, or objects with {name, icon/src})
 * @param {Object} centerIcon - Center icon (optional, defaults to first tech or a default icon)
 * @param {string} className - Additional classes for container
 */
export function TechStacks({ techs = [], centerIcon = null, className }) {
